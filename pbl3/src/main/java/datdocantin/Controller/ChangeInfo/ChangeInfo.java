	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try { 
			if (session.getAttribute("khachhang") != null) {
					...
					if (avatarBytes.length == 0) avatarBytes = null;
					if (AccountDAO.CheckAccountNotExist(sdt)) {
						CanteenDAO.updateInfo(new CanteenModel(canteen.getID_canteen(), ten, sdt, email, null, null, avatarBytes, null));
						AccountDAO.ChangeSdt(canteen.getID_canteen(), sdt);
					} else {
						CanteenDAO.updateInfo(new CanteenModel(canteen.getID_canteen(), ten, null, email, null, null, avatarBytes, null));
					}
